"""
database.py - Live Feed Progress Engine & Formlabs SCADA Data Layer
"""
...
UPLOAD_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "uploads", "cleanliness")
...
def seed_initial_data():
    ...
    try:
        if session.query(User).count() == 0:
            users = [
                User(username="operator", pin="1234", full_name="Demo Operator", role="operator"),
                User(username="sasha", pin="1234", full_name="Sasha", role="operator"),
                User(username="manager", pin="admin", full_name="Plant Lead", role="manager"),
            ]
        ...
        # ONLY seeds generic defaults if the resin table is completely empty
        if session.query(ResinSpec).count() == 0:
            for (cart_type, sku_val, code_val, life_val, name_val, weight_val, min_w, max_w, range_val, mult_val, color_val) in MASTER_FORMLABS_CATALOG:
                spec = ResinSpec(
                    ...
                    max_weight_g=float(max_w), acceptable_range=range_val, multiplier=float(mult_val), color_tag=color_val
                )
                session.add(spec)
...
def calculate_logged_units_for_resin(resin_name: str, cartridge_type: str = "", pump_station: str = "", lot_number: str = "") -> int:
    session = ScopedSession()
    try:
...
def sync_all_runs_with_logs():
    ...
    try:
        ...
        for r in runs:
            actual_logged = calculate_logged_units_for_resin(r.resin_type, r.cartridge_type, r.pump_station, r.lot_number)
...
def authenticate_user(username: str, pin: str):
    session = ScopedSession()
    try:
        # Hash the incoming PIN to compare against the database
        hashed_pin = hashlib.sha256(pin.strip().encode()).hexdigest()

        user = session.query(User).filter(User.username == username.lower().strip(), User.pin == hashed_pin).first()
        if user:
            return {
                "id": user.id,
...
def update_user_pin(user_id: int, new_pin: str) -> bool:
    ...
    try:
        ...
        if user:
            ...
            user.pin = hashlib.sha256(new_pin.strip().encode()).hexdigest()
            session.commit()
            return True
        return False
    finally:
...
def reconcile_reactor_level(reactor_name: str, visual_fill_pct: float, operator_name: str, notes: str = "") -> bool:
    ...
    try:
        ...
        vol_mult = 1.0
        active_run = session.query(AssignedRun).filter(AssignedRun.reactor_id == reactor_name, AssignedRun.status.in_(["Active", "Pouring"])).first()
        ...
        current_logged = calculate_logged_units_for_resin(resin_name=reactor.current_resin, pump_station=reactor.assigned_pump or "")
        
        session.add(ProductionLog(
            log_type="Hourly Bottle Count", operator_name=operator_name, pump_station=reactor.assigned_pump or "Visual Check",
            shift="Shift 1", cartridge_type="V2" if vol_mult == 1.0 else "RPS", resin_type=reactor.current_resin,
            lot_number=(active_run.lot_number if active_run else "RECON-ADJ"), bottles_filled=target_units_poured - current_logged,
...
def add_downtime_log(operator_name: str, pump_station: str, shift: str, reason: str, duration_min: int, notes: str = ""):
    session = ScopedSession()
    try:
        session.add(DowntimeLog(operator_name=operator_name, pump_station=pump_station, shift=shift, reason=reason, duration_min=duration_min, notes=notes))
        session.commit()
    finally:
        session.close()

def add_cleanliness_audit(audit_type: str, operator_name: str, pump_station: str, shift: str, resin_type: str, notes: str, is_spill: bool, uploaded_file=None) -> bool:
    session = ScopedSession()
    try:
        saved_filename = None
        ...
        session.add(CleanlinessAudit(audit_type=audit_type, operator_name=operator_name, pump_station=pump_station, shift=shift, resin_type=resin_type if resin_type else None, image_filename=saved_filename, is_spill="Yes" if is_spill else "No", notes=notes))
        session.commit()
        return True
    except Exception as e:
        session.rollback()
...
def get_cleanliness_audits_df() -> pd.DataFrame:
    session = ScopedSession()
    try: return pd.read_sql(session.query(CleanlinessAudit).order_by(desc(CleanlinessAudit.timestamp)).statement, session.bind)
...
def get_downtime_logs_df() -> pd.DataFrame:
    session = ScopedSession()
    try: return pd.read_sql(session.query(DowntimeLog).order_by(desc(DowntimeLog.timestamp)).statement, session.bind)
    finally: session.close()
...
def send_floor_message(operator_name: str, sender_name: str, message: str, is_manager: bool):
    session = ScopedSession()
    try:
        session.add(FloorMessage(operator_name=operator_name, sender_name=sender_name, message=message, is_manager_reply=1 if is_manager else 0))
        session.commit()
    finally: session.close()

def get_chat_history_df(operator_name: str) -> pd.DataFrame:
    session = ScopedSession()
    try: return pd.read_sql(session.query(FloorMessage).filter(FloorMessage.operator_name == operator_name).order_by(FloorMessage.timestamp).statement, session.bind)
    finally: session.close()

def get_operators_with_messages() -> list:
    session = ScopedSession()
    try: return [row[0] for row in session.query(FloorMessage.operator_name).distinct().all()]
    finally: session.close()
...
def update_user_credentials(user_id: int, new_username: str = None, new_pin: str = None, new_fullname: str = None) -> tuple[bool, str]:
    session = ScopedSession()
    try:
        user = session.query(User).filter(User.id == user_id).first()
        if not user:
