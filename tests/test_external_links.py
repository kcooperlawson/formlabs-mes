...
# "the documents the app hands people" used to check here that the question
# mark on the operator form and the handbook line in the manager menu pointed
# at real PDFs under static/, served because .streamlit/config.toml turned
# Streamlit's static serving on. All of it - static/, .streamlit/, and the
# pages that linked to them - retired with the Streamlit UI. The React app's
# equivalent help links (if/when it grows them) will need their own version
# of this check, pointed at wherever it serves documents from instead.
